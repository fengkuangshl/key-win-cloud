...
// import activitiModdleDescriptor from '@/common/bpmn/activiti.json'
// 汉化
import customTranslate from '@/common/bpmn/customTranslate'
@Component({
  components: {}
})
export default class BpmnJs extends Vue {
  ...
  @Ref('downloadLink')
  readonly downloadLink!: HTMLAnchorElement
  ...
  downloadSvg(): void {
    this.bpmnModeler.saveXML({ format: true }, (err: any, xml: string) => {
      if (!err) {
        ...
        // 从建模器画布中提取svg图形标签
        let context = ''
        const djsGroupAll = this.canvas.querySelectorAll('.djs-group')
        for (const item of djsGroupAll) {
          context += item.innerHTML
        }
        // 获取svg的基本数据，长宽高
        const viewport = (this.canvas.querySelector('.viewport') as SVGSVGElement).getBBox()
  ...
  openBpmn(file: Blob): boolean {
    const reader = new FileReader()
    // 读取File对象中的文本信息，编码格式为UTF-8
    reader.readAsText(file, 'utf-8')
    reader.onload = () => {
      // 读取完毕后将文本信息导入到Bpmn建模器
      this.createNewDiagram(reader.result as string)
    }
    return false
  }

  getFilename(xml: string): string {
  ...
  download(rs: { name: string; data?: string }): void {
    // 这里就获取到了之前设置的隐藏链接
    const downloadLink = this.downloadLink

    if (rs.data != null) {
      // 把输就转换为URI，下载要用到的
      const encodedData = encodeURIComponent(rs.data)
      // 将数据给到链接
      downloadLink.href = 'data:application/bpmn20-xml;charset=UTF-8,' + encodedData
      // 设置文件名
      downloadLink.download = rs.name === null ? 'diagram.bpmn' : rs.name
      // 触发点击事件开始下载
      downloadLink.click()
    }
  }
  ...
  async createNewDiagram(bpmn: string): Promise<void> {
    // 将字符串转换成图显示出来;
    this.bpmnModeler.importXML(bpmn, (err: any) => {
      if (err) {
