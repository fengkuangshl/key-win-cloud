  ...
  <div>
    <el-form :model="dynamicFormData" ref="dynamicFormRef" :inline='false' :rules="dynamicFormRules" label-width="auto" class="dynamicForm">
      <el-form-item v-for="(item,index) in formItems" :key="index" :label="item.label+'：'" :prop="item.model">
        <el-input v-if="item.type == null || item.type == undefined || item.type == 'text'" :placeholder="'请填写'+item.label" v-model="dynamicFormData[item.model]" style="max-width: 220px;"></el-input>
        ...
        <el-select v-else-if="item.type == 'select'" :placeholder="'请选择'+item.label" v-model="dynamicFormData[item.model]" @change="item.isFun ? onAction(dynamicFormData[item.model]) : ''" style="max-width: 220px;">
