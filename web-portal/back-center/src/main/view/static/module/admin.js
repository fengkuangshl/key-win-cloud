    ...
    var admin = {
        ...
        activeNav: function (url) {
            ...
            if (url && url != '') {
                ...
                if($a.length == 0){
                    $a = $('.layui-layout-admin .layui-side .layui-nav>.layui-nav-item>.layui-nav-child>dd>.layui-nav-child>dd>a[href="#!' + url + '"]');
                    $a.parent('dd').addClass('layui-this');
                    $a.parent('dd').parent('ol').parent('dd').addClass('layui-nav-itemed');
                    //$a.parent('li').addClass('layui-this');
                    $a.parent('dd').parent('ol').parent('dd').parent('dl').parent('li').addClass('layui-nav-itemed');
        ...
        req: function (url, data, success, method, isAlert) {
...
//                	}
        ...
        post : function(url, data, success){
            ...
            admin.req(url, data, success,"POST");
        },
        get : function(url, data, success){
            admin.req(url, data, success,"GET");
        },
        put : function(url, data, success){
            admin.req(url, data, success,"PUT");
        },
        delete : function(url, data, success){
            admin.req(url, data, success,"DELETE");
        },
        postAlert : function(url, data, success){
            data = JSON.stringify(data);
            admin.req(url, data, success,"POST",true);
        },
        getAlert : function(url, data, success){
            admin.req(url, data, success,"GET",true);
        },
        putAlert : function(url, data, success){
            admin.req(url, data, success,"PUT",true);
        },
        deleteAlert : function(url, data, success){
            admin.req(url, data, success,"DELETE",true);
        },
        asyncReq: function (url, data, success, method,isAlert) {
            if ('put' == method.toLowerCase()) {
                // method = 'POST';
                // data._method = 'PUT';
                method = 'PUT';
